...
export const EditTagModal: React.FC<EditTagModalProps> = ({ oldTagName, onClose }) => {
    const [newTagName, setNewTagName] = useState(oldTagName);
    const { renameTag } = useAppStore();

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const trimmed = newTagName.trim();
        if (trimmed && trimmed !== oldTagName) {
            renameTag(oldTagName, trimmed);
            onClose();
        } else if (trimmed === oldTagName) {
            onClose();
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-sm bg-gray-900 border border-gray-700 rounded-lg shadow-2xl p-6 relative">
                ...
                <h3 className="text-lg font-semibold text-white mb-4">Đổi tên thẻ</h3>

                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">Tên mới</label>
                        <input
                            type="text"
                            value={newTagName}
